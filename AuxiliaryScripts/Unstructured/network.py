...
from AuxiliaryScripts.Unstructured import utils
    ...
    def reinit_statedict(self, tasknum:int):
        for module_idx, module in enumerate(self.model.shared.modules()):
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                ...
                frozen_weights = utils.get_frozen_mask(module.weight.data, module_idx, self.all_task_masks, tasknum)
                new_weights[frozen_weights.eq(1)] = 0
